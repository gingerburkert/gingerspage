...
* http://jqueryui.com
* Includes: core.js, widgetck.js, accordion.js, tabs.js
 ...
 * http://jquery.org/license
 *
 * http://api.jqueryui.com/jQuery.widgetck/
 */


var widgetck_uuid = 0,
	widgetck_slice = Array.prototype.slice;
...
$.widgetck.extend = function( target ) {
	var input = widgetck_slice.call( arguments, 1 ),
		inputIndex = 0,
...
$.widgetck.bridge = function( name, object ) {
	...
	$.fn[ name ] = function( options ) {
		var isMethodCall = typeof options === "string",
			args = widgetck_slice.call( arguments, 1 ),
			returnValue = this;
		...
		if ( isMethodCall ) {
			...
			if ( args.length ) {
				options = $.widgetck.extend.apply( null, [ options ].concat(args) );
			}
...
$.Widget.prototype = {
	...
	_setOption: function( key, value ) {
		...
		if ( key === "disabled" ) {
			this.widgetck()
				.toggleClass( this.widgetckFullName + "-disabled", !!value );

			// If the widgetck is becoming disabled, then nothing is interactive
			if ( value ) {
				this.hoverable.removeClass( "ui-state-hover" );
				this.focusable.removeClass( "ui-state-focus" );
			}
...
var accordionck = $.widgetck( "uick.accordionck", {
	version: "1.11.4",
	options: {
		active: 0,
		animate: {},
	...
	_destroy: function() {
		...
		contents = this.headers.next()
			.removeClass( "ui-helper-reset ui-widgetck-content ui-corner-bottom " +
	...
	_animate: function( toShow, toHide, data ) {
		var total, easing, duration,
			...
			scrollToActive = this.options.scrollToActive,
			complete = function() {
				if (scrollToActive) $('html, body').animate({scrollTop: toShow.offset().top - 30}, 'slow');
...
var tabsck = $.widgetck( "uick.tabsck", {
	version: "1.11.4",
	delay: 300,
	options: {
		active: null,
	...
	_processTabs: function() {
		...
		this.anchors.each(function( i, anchor ) {
			...
			// inline tab
			// hack by ced1870 : removed to avoid conflicts with base tag, assume that all tabs are local
