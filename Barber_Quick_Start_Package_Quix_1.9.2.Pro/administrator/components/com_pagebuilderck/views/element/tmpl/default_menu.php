...
<?php
// $standarditems = array('row', 'text', 'icon', 'icontext', 'image', 'separator', 'message', 'tabs', 'accordion', 'module', 'video', 'audio', 'readmore');
...
<div id="menuck" class="menuck clearfix ckinterface">
	<div class="inner clearfix ckelementsedition">
		...
		// $items = $otheritems;
		$typesImagesArray = array();
		if (count($items)) {
			foreach ($items as $item) {
				$typesImagesArray[$item->type] = $item->image;
				?>
		...
		?>
		<div class="" tab="ckmyelements"><?php echo JText::_('CK_MY_ELEMENTS'); ?></div>
			...
			if (count($this->elements)) {
				foreach ($this->elements as $element) {
					...
					<div data-type="<?php echo $element->type ?>" data-id="<?php echo $element->id ?>" class="menuitemck ckmyelement" title="<b><?php echo $description ?></b>">
						...
						<img src="<?php echo $typesImagesArray[$element->type] ?>" />
					</div>
					<?php
				}
			}
